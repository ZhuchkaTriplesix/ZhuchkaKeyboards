...
**Directory:** эндпоинты **`/api/v1/me`**, **`/api/v1/me/addresses`**, **`/api/v1/me/consents`** ожидают **Bearer** access token от Auth; операционные **`GET /api/v1/customers`** и **`GET /api/v1/customers/{id}`** — тот же Bearer, но в JWT в **`scope`** должен быть **`admin`**. В `docker/directory/config.dev.ini` секция **`[AUTH]`** (JWKS, `ISSUER`, `AUDIENCE`) согласована с **`docker/auth/config.dev.ini`**.
