...
class TestWarehouseAPI:
    ...
    def test_create_warehouse_success(self, mock_app: TestClient, sample_warehouse_data):
        ...
        with patch('routers.inventory.crud.get_warehouse_by_code') as mock_get, \
            ...
            response = mock_app.post("/api/inventory/warehouses", json=sample_warehouse_data)
    ...
    def test_create_warehouse_duplicate_code(self, mock_app: TestClient, sample_warehouse_data):
...
class TestItemAPI:
    ...
    def test_get_items_with_filters(self, mock_app: TestClient):
        ...
        with patch('routers.inventory.crud.search_items') as mock_crud:
            ...
            response = mock_app.get("/api/inventory/items?item_type=component&search=test")
...
class TestInventoryLevelAPI:
    ...
    def test_create_inventory_level_success(self, mock_app: TestClient, sample_inventory_level_data):
        """Test POST /api/inventory/inventory - success"""
        mock_level = create_mock_inventory_level(**sample_inventory_level_data)
        
        with patch('routers.inventory.crud.get_inventory_level') as mock_get, \
            ...
            response = mock_app.post("/api/inventory/inventory", json=sample_inventory_level_data)
            
            assert response.status_code == status.HTTP_201_CREATED
            data = response.json()
            assert data["current_quantity"] == sample_inventory_level_data["current_quantity"]
