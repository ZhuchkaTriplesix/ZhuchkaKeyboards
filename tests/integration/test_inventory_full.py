...
class TestInventoryIntegration:
    ...
    def test_complete_item_lifecycle(self, integration_app: TestClient):
        ...
        response = integration_app.put(
            f"/api/inventory/items/{item_id}", json=update_data
        )
        assert response.status_code == status.HTTP_200_OK
        ...
        response = integration_app.get(
            "/api/inventory/items?item_type=component&category=switches"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_inventory_level_and_movements(self, integration_app: TestClient):
        ...
        response = integration_app.get(
            f"/api/inventory/inventory?warehouse_id={warehouse_id}"
        )
        assert response.status_code == status.HTTP_200_OK
        ...
        response = integration_app.post(
            "/api/inventory/reserve", params=over_reserve_params
        )
    ...
    def test_analytics_endpoints(self, integration_app: TestClient):
        ...
        item_response = integration_app.post(
            "/api/inventory/items", json=low_stock_item
        )
        item_id = item_response.json()["id"]
        ...
        response = integration_app.get(
            f"/api/inventory/analytics/summary?warehouse_id={warehouse_id}"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_error_handling(self, integration_app: TestClient):
        ...
        response = integration_app.get(
            f"/api/inventory/inventory/{non_existent_id}/{non_existent_id}"
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
