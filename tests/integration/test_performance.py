...
class TestInventoryPerformance:
    ...
    def test_item_search_performance(self, integration_app: TestClient):
        ...
        search_scenarios = [
            ...
            {"params": "?search=Item&item_type=component", "description": "Combined filters"},
        ...
        for scenario in search_scenarios:
            ...
            print(f"{scenario['description']}: {response_time:.3f}s ({results['total']} results)")
    ...
    def test_concurrent_stock_movements(self, integration_app: TestClient):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(perform_stock_movement, i) for i in range(concurrent_movements)]
        ...
        print(f"Final quantity: {final_level['current_quantity']}, Expected: {expected_quantity}")
        
        # Assertions
        assert successful_movements > 0  # At least some should succeed
        assert final_level["current_quantity"] == expected_quantity  # Quantity should be consistent
    ...
    def test_reservation_performance(self, integration_app: TestClient):
        ...
        print(f"Reservation performance:")
        print(f"  Total reservations: {reservations_to_make}")
        print(f"  Reservation time: {reservation_time:.2f}s ({avg_reservation_time:.3f}s avg)")
    ...
    def test_analytics_performance(self, integration_app: TestClient):
        ...
        for item in items[:100]:  # Only for first 100 items
            for warehouse in warehouses[:5]:  # Only for first 5 warehouses
                current_qty = 5 if item["min_stock_level"] == 50 else 100  # Create low stock
                level_data = {
                    "item_id": item["id"],
