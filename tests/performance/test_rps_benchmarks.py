...
class TestRPSBenchmarks:
    ...
    async def test_empty_database_baseline(self):
        ...
        stats = self._analyze_results("Empty Database Baseline", results, total_test_time, target_rps)
        
        # Базовая производительность должна быть высокой
        assert stats["success_rate"] > 0.8, f"Baseline success rate too low: {stats['success_rate']:.2f}"
        assert stats["actual_rps"] > 50, f"Baseline RPS too low: {stats['actual_rps']:.2f}"
    ...
    async def test_loaded_database_performance(self):
        ...
        stats = self._analyze_results("Loaded Database", results, total_test_time, target_rps)
        
        # Производительность с данными должна быть приемлемой
        assert stats["success_rate"] > 0.7, f"Loaded DB success rate too low: {stats['success_rate']:.2f}"
        assert stats["actual_rps"] > 30, f"Loaded DB RPS too low: {stats['actual_rps']:.2f}"
    ...
    async def test_heavy_database_stress(self):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            for i in range(total_requests):
                ...
                if "search=" in endpoint and random.random() < 0.3:
                    search_terms = ["key", "switch", "cap", "component", "mechanical", "board"]
        ...
        stats = self._analyze_results("Heavy Database Stress", results, total_test_time, target_rps)
        
        # Даже под нагрузкой должна быть минимальная производительность
        assert stats["success_rate"] > 0.6, f"Heavy DB success rate too low: {stats['success_rate']:.2f}"
        assert stats["actual_rps"] > 10, f"Heavy DB RPS too low: {stats['actual_rps']:.2f}"
    ...
    async def test_mixed_operations_benchmark(self):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            for i in range(total_requests):
                ...
                if operation == "GET":
                    ...
                    task = self._make_request(
                        session, "PUT", f"{BASE_URL}/api/inventory/warehouses/{resource_id}",
                        json=update_data
                    )
        ...
        stats = self._analyze_results("Mixed Operations", results, total_test_time, target_rps)
        
        # Смешанные операции должны работать стабильно
        assert stats["success_rate"] > 0.6, f"Mixed operations success rate too low: {stats['success_rate']:.2f}"
        assert stats["actual_rps"] > 20, f"Mixed operations RPS too low: {stats['actual_rps']:.2f}"
    ...
    async def test_concurrent_users_simulation(self):
        ...
        connector = aiohttp.TCPConnector(limit=users_count*2, limit_per_host=users_count*2)
        timeout = aiohttp.ClientTimeout(total=30)
        ...
        stats = self._analyze_results("Concurrent Users", all_results, total_test_time, target_rps=0)
        
        print(f"  Simulated users: {users_count}")
        print(f"  Requests per user: {requests_per_user}")
        print(f"  User think time: 0.1-1.0s between requests")
        
        # Многопользовательский режим должен работать стабильно
        assert stats["success_rate"] > 0.7, f"Multi-user success rate too low: {stats['success_rate']:.2f}"
    ...
    def _analyze_results(self, test_name: str, results: List, total_time: float, target_rps: int) -> Dict:
        ...
        if target_rps > 0:
            if actual_rps >= target_rps * 0.8:
                ...
                print(f"  🟠 Performance Rating: Fair")
            else:
    ...
    async def test_performance_comparison_report(self):
        ...
        benchmark_results["concurrent_users"] = await self.test_concurrent_users_simulation()
        ...
        if "empty_db" in benchmark_results and "loaded_db" in benchmark_results:
            ...
            degradation = ((empty_rps - loaded_rps) / empty_rps * 100) if empty_rps > 0 else 0
            print(f"  • Performance degradation with data: {degradation:.1f}%")
        
        print(f"\n🚀 OPTIMIZATION RECOMMENDATIONS:")
        print(f"  1. Database indexing for search operations")
        print(f"  2. Redis caching for frequently accessed data")
        print(f"  3. Connection pooling optimization")
        print(f"  4. Load balancing for concurrent users")
        print(f"  5. Query optimization for analytics endpoints")
