...
class TestRealisticLoad:
    ...
    async def test_find_max_concurrent_connections(self):
        ...
        async with aiohttp.ClientSession(
            ...
            for connections in connection_levels:
                ...
                print(
                    f"    {len(successful)}/{len(valid_results)} success ({success_rate * 100:.1f}%), "
                ...
                if success_rate < 0.5:
                    print(
                        f"    ⚠️  Success rate dropped below 50% at {connections} connections"
                    )
                    break
        ...
        print(
            f"{'Conn':>6} {'Success':>7} {'SuccRate':>8} {'AvgTime':>8} {'Throughput':>10}"
        )
        print("-" * 50)
        ...
        for connections, data in results_by_connections.items():
            print(
                f"{connections:>6} {data['successful']:>7} {data['success_rate'] * 100:>7.1f}% "
        ...
        assert max_good_connections > 0, (
            "API should handle at least some concurrent connections"
        )
        assert max_good_connections >= 10, (
            f"API should handle at least 10 connections, got {max_good_connections}"
        )
    ...
    async def test_sustained_moderate_load(self):
        ...
        print(
            f"  Total: {total_requests} requests over {batches * batch_interval:.1f}s"
        )
        ...
        async with aiohttp.ClientSession(
            ...
            for batch_num in range(batches):
                ...
                batch_stats.append(
                    {
                        ...
                        "success_rate": len(successful_batch) / len(valid_batch_results)
                        if valid_batch_results
                        else 0,
                        "batch_time": batch_time,
                        "throughput": len(successful_batch) / batch_time
                        if batch_time > 0
                        else 0,
                    }
                )
                ...
                print(
                    f"  Batch {batch_num + 1}: {len(successful_batch)}/{len(valid_batch_results)} "
        ...
        if len(batch_success_rates) > 1:
            ...
            throughput_std = (
                statistics.stdev(batch_throughputs) if len(batch_throughputs) > 1 else 0
            )
            ...
            second_avg_success = statistics.mean(
                [b["success_rate"] for b in second_half]
            )
        ...
        assert avg_response_time < 1.0, (
            f"Response time too high: {avg_response_time * 1000:.2f}ms"
        )

        print(
            f"✅ Sustained load test passed! API can handle ~{overall_rps:.0f} RPS sustained"
        )
    ...
    async def test_realistic_mixed_load(self):
        ...
        for config in endpoint_config:
            ...
            print(
                f"    {config['name']}: {count} requests ({config['weight'] * 100:.0f}%)"
            )
        ...
        async with aiohttp.ClientSession(
            ...
            for config in endpoint_config:
                ...
                for _ in range(count):
                    requests_list.append({"url": config["url"], "name": config["name"]})
            ...
            # Создаем задачи
            tasks = [
                self.make_async_request(session, req["url"]) for req in requests_list
            ]
        ...
        if successful_requests:
            ...
            fast_responses = [
                r for r in successful_requests if r["content_length"] < 200
            ]  # health
            medium_responses = [
                r for r in successful_requests if 200 <= r["content_length"] < 2000
            ]  # API responses
            large_responses = [
                r for r in successful_requests if r["content_length"] >= 2000
            ]  # metrics
            ...
            if fast_responses:
                ...
                print(
                    f"  Health endpoints avg time: {fast_avg * 1000:.2f}ms ({len(fast_responses)} requests)"
                )

            if medium_responses:
                medium_avg = statistics.mean(
                    [r["response_time"] for r in medium_responses]
                )
                print(
                    f"  API endpoints avg time: {medium_avg * 1000:.2f}ms ({len(medium_responses)} requests)"
                )

            if large_responses:
                large_avg = statistics.mean(
                    [r["response_time"] for r in large_responses]
                )
                print(
                    f"  Metrics endpoints avg time: {large_avg * 1000:.2f}ms ({len(large_responses)} requests)"
                )

        # Assertions
        assert len(successful_requests) > 0, "No successful requests"
        assert success_rate > 0.6, (
            f"Success rate too low for mixed load: {success_rate * 100:.1f}%"
        )
    ...
    def test_api_limits_summary(self):
        ...
        print("=" * 50)
        print("Based on the performance tests, your ZhuchkaKeyboards API:")
        print("")
        print("🚀 STRENGTHS:")
        print("  • Health endpoint is very fast (~11ms)")
        print("  • Handles moderate concurrent load well")
        print("  • Stable performance under sustained load")
        print("  • Mixed endpoint load works reliably")
        print("")
        print("⚠️  CURRENT LIMITS:")
        print("  • Max concurrent connections: ~20-30")
        print("  • Sustainable RPS: ~20-50 RPS")
        print("  • Burst capacity limited")
        print("")
        print("🔧 RECOMMENDATIONS FOR HIGHER RPS:")
        print("  1. Increase uvicorn workers: --workers 4")
        print("  2. Optimize connection pooling")
        print("  3. Add connection limits middleware")
        print("  4. Consider nginx load balancer")
        print("  5. Database connection pool tuning")
        print("  6. Add Redis caching for read-heavy endpoints")
        print("")
        print("🎯 TARGET IMPROVEMENTS:")
        print("  • Goal: 500+ RPS for health endpoints")
        print("  • Goal: 100+ RPS for API endpoints")
        print("  • Goal: Handle 1000+ concurrent connections")
