...
class TestDataLoader:
    ...
    def __init__(self, base_url: str = BASE_URL):
        ...
        self.loaded_resources = {"warehouses": [], "items": [], "suppliers": []}
        self.stats = {
            "total_requests": 0,
    ...
    async def load_warehouses(
        ...
        print(
            f"📦 Loading {len(warehouses_data)} warehouses in batches of {batch_size}..."
        )

        connector = aiohttp.TCPConnector(
            limit=batch_size * 2, limit_per_host=batch_size * 2
        )
        ...
        async with aiohttp.ClientSession(
            ...
            for i in range(0, len(warehouses_data), batch_size):
                ...
                print(
                    f"  Batch {i // batch_size + 1}/{(len(warehouses_data) - 1) // batch_size + 1}: {len(batch)} warehouses"
                )
                    ...
                    if isinstance(result, dict) and result.get("success"):
                        ...
                        if isinstance(result, dict):
                            self.stats["errors"].append(
                                f"Warehouse: {result.get('error', 'Unknown error')}"
                            )
                        else:
                ...
                print(
                    f"    ✅ {successful_in_batch}/{len(batch)} successful, {batch_rps:.1f} RPS"
                )
        ...
        print(
            f"📦 Warehouses loading complete: {len(loaded_warehouses)}/{len(warehouses_data)} loaded"
        )
    ...
    async def load_items(
        ...
        async with aiohttp.ClientSession(
            ...
            for i in range(0, len(items_data), batch_size):
                ...
                print(
                    f"  Batch {i // batch_size + 1}/{(len(items_data) - 1) // batch_size + 1}: {len(batch)} items"
                )
        ...
        print(
            f"🔧 Items loading complete: {len(loaded_items)}/{len(items_data)} loaded"
        )
        return loaded_items

    async def _create_warehouse(
        self, session: aiohttp.ClientSession, warehouse_data: Dict
    ) -> Dict:
        ...
        try:
            async with session.post(
                ...
                if 200 <= response.status < 300:
                    ...
                    return {
                        "success": False,
                        "error": f"Status {response.status}: {error_text[:200]}",
                    }
    ...
    async def _create_item(
        self, session: aiohttp.ClientSession, item_data: Dict
    ) -> Dict:
...
async def load_small_dataset():
    ...
    await loader.load_items(
        items[:50], batch_size=10, delay_between_batches=0.2
    )  # Только 50 items для small
...
async def load_medium_dataset():
    ...
    await loader.load_items(
        items[:500], batch_size=20, delay_between_batches=0.2
    )  # 500 items для medium
...
async def load_large_dataset():
    ...
    await loader.load_items(
        items[:2000], batch_size=30, delay_between_batches=0.1
    )  # 2000 items для large
    ...
    total_records = len(loader.loaded_resources["warehouses"]) + len(
        loader.loaded_resources["items"]
    )
    if total_time > 0:
