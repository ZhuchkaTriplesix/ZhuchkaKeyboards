...
class TestHighRPSPerformance:
    ...
    async def test_health_endpoint_high_rps(self):
        ...
        print(f"  Success rate: {len(successful_requests)/len(all_results)*100:.2f}%")
        ...
        assert len(successful_requests) / len(all_results) > 0.95, f"Success rate too low: {len(successful_requests)/len(all_results)*100:.2f}%"
        assert avg_response_time < 0.1, f"Average response time too high: {avg_response_time*1000:.2f}ms"
        assert actual_rps > target_rps * 0.8, f"RPS too low: {actual_rps:.2f} (target: {target_rps})"
    ...
    async def test_warehouse_list_high_rps(self):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            tasks = [
                self.make_async_request(session, "GET", f"{BASE_URL}/api/inventory/warehouses")
        ...
        print(f"📊 Warehouse listing results:")
    ...
    async def test_mixed_endpoints_load(self):
        ...
        endpoints = [
            ...
            {"method": "GET", "url": f"{BASE_URL}/api/inventory/warehouses", "weight": 0.25},
            {"method": "GET", "url": f"{BASE_URL}/api/inventory/items", "weight": 0.25},
            {"method": "GET", "url": f"{BASE_URL}/api/inventory/analytics/summary", "weight": 0.15},
            {"method": "GET", "url": f"{BASE_URL}/api/inventory/analytics/low-stock", "weight": 0.05},
        ]
    ...
    async def test_gradual_ramp_up(self):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            for target_rps in rps_levels:
                ...
                print(f"    Result: {actual_rps:.1f} RPS, {success_rate*100:.1f}% success, {avg_response_time*1000:.1f}ms avg")
                
                # Если производительность сильно упала, прерываем тест
                if success_rate < 0.8 or avg_response_time > 0.5:
                    print(f"    ⚠️  Performance degraded significantly at {target_rps} RPS")
                    break
        ...
        # Итоговый анализ
        print(f"\n📊 RPS Ramp-up Summary:")
        print(f"{'RPS':>6} {'Actual':>8} {'Success%':>8} {'AvgTime':>8} {'Requests':>9}")
        print("-" * 50)
        
        max_stable_rps = 0
        for target_rps, data in results_by_rps.items():
            print(f"{target_rps:>6} {data['actual_rps']:>8.1f} {data['success_rate']*100:>7.1f}% {data['avg_response_time']*1000:>7.1f}ms {data['successful_requests']:>9}")
        ...
        assert max_stable_rps >= 500, f"API should handle at least 500 RPS, but max stable was {max_stable_rps:.1f}"
