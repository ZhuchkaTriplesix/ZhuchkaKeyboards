...
class TestMetricsPerformance:
    ...
    async def test_metrics_collection_under_load(self):
        ...
        async with aiohttp.ClientSession(
            ...
            successful_api_requests = len(
                [r for r in api_results if isinstance(r, dict) and r.get("success")]
            )
            ...
            print(
                f"  Accuracy: {metrics_increase / successful_api_requests * 100:.1f}%"
            )
            ...
            assert (
                abs(metrics_increase - successful_api_requests)
                / successful_api_requests
                < 0.1
            )

    async def get_request_count_from_metrics(
