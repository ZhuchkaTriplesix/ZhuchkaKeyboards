...
from tests.performance.data_generators import generate_small_dataset, generate_medium_dataset
...
class TestAllMethodsRPS:
    ...
    async def test_warehouse_get_rps(self):
        ...
        self._analyze_and_print_results("Warehouse GET", valid_results, successful_requests, total_test_time, target_rps)
        
        # Assertions
        assert len(successful_requests) > 0, "No successful requests"
        assert len(successful_requests) / len(valid_results) > 0.7, f"Success rate too low"
    ...
    async def test_warehouse_post_rps(self):
        ...
        self._analyze_and_print_results("Warehouse POST", valid_results, successful_requests, total_test_time, target_rps)
        ...
        if failed_requests:
            ...
            for i, error in enumerate(failed_requests[:3]):
                print(f"    {i+1}. Status {error['status_code']}: {str(error['error'])[:100]}...")
    ...
    async def test_items_get_rps(self):
        ...
        self._analyze_and_print_results("Items GET", valid_results, successful_requests, total_test_time, target_rps)
        
        # Assertions
        assert len(successful_requests) > 0, "No successful requests"
        assert len(successful_requests) / len(valid_results) > 0.6, f"Success rate too low for items GET"
    ...
    async def test_items_post_rps(self):
        ...
        self._analyze_and_print_results("Items POST", valid_results, successful_requests, total_test_time, target_rps)
    ...
    async def test_mixed_crud_operations_rps(self):
            ...
            for i in range(total_requests):
                ...
                if operation == "GET":
                    ...
                    if random.choice([True, False]):  # 50% warehouses, 50% items
                        ...
                        task = self.make_async_request(
                            session, "POST", f"{BASE_URL}/api/inventory/warehouses",
                        ...
                        task = self.make_async_request(
                            session, "POST", f"{BASE_URL}/api/inventory/items",
                            json=data, headers={"Content-Type": "application/json"}
                        )
                
                elif operation == "PUT":
                    # PUT operations (если есть созданные ресурсы)
                    if self.created_resources["warehouses"] and random.choice([True, False]):
                        warehouse_id = random.choice(self.created_resources["warehouses"])
                        update_data = {"name": f"Updated Warehouse {i}"}
                        task = self.make_async_request(
                            session, "PUT", f"{BASE_URL}/api/inventory/warehouses/{warehouse_id}",
                        ...
                        task = self.make_async_request(
                            session, "PUT", f"{BASE_URL}/api/inventory/items/{item_id}",
                            json=update_data, headers={"Content-Type": "application/json"}
                        )
                    else:
                        # Fallback to GET if no resources to update
                        task = self.make_async_request(session, "GET", f"{BASE_URL}/api/health")
                
                else:  # DELETE
                    # DELETE operations (осторожно с удалением)
                    if self.created_resources["items"] and len(self.created_resources["items"]) > 5:
                        item_id = self.created_resources["items"].pop()  # Удаляем и убираем из списка
                        task = self.make_async_request(
        ...
        self._analyze_and_print_results("Mixed CRUD", valid_results, successful_requests, total_test_time, target_rps)
        print(f"  Status code distribution: {status_codes}")
        
        assert len(successful_requests) > 0, "No successful requests"
        assert len(successful_requests) / len(valid_results) > 0.5, f"Success rate too low for mixed operations"
    ...
    async def test_analytics_endpoints_rps(self):
        ...
        if successful_requests:
            ...
            small_responses = [r for r in successful_requests if r["content_length"] < 1000]
            large_responses = [r for r in successful_requests if r["content_length"] >= 1000]
            
            print(f"  Small responses (<1KB): {len(small_responses)}")
            print(f"  Large responses (≥1KB): {len(large_responses)}")
        
        self._analyze_and_print_results("Analytics", valid_results, successful_requests, total_test_time, target_rps)
        
        assert len(successful_requests) > 0, "No successful analytics requests"
    
    def _analyze_and_print_results(self, test_name: str, valid_results: List[Dict], 
                                  successful_requests: List[Dict], total_test_time: float, target_rps: int):
    ...
    async def test_rps_summary_report(self):
        ...
        print(f"  • Test data: {len(self.test_data.get('warehouses', []))} warehouses, {len(self.test_data.get('items', []))} items")
        print(f"  • Created resources during tests:")
        print(f"    - Warehouses: {len(self.created_resources['warehouses'])}")
        print(f"    - Items: {len(self.created_resources['items'])}")
        print(f"")
        print(f"🚀 API METHOD PERFORMANCE TARGETS:")
        print(f"  • GET /warehouses: 200 RPS target")
        print(f"  • POST /warehouses: 50 RPS target")
        print(f"  • GET /items: 150 RPS target")
        print(f"  • POST /items: 30 RPS target")
        print(f"  • Mixed CRUD: 100 RPS target")
        print(f"  • Analytics: 50 RPS target")
        print(f"")
        print(f"📊 EXPECTED PERFORMANCE CHARACTERISTICS:")
        print(f"  • GET operations: Fastest (DB reads + caching)")
        print(f"  • POST operations: Slower (validation + DB writes)")
        print(f"  • PUT operations: Medium (partial updates)")
        print(f"  • DELETE operations: Fast (simple operations)")
        print(f"  • Analytics: Variable (complex queries)")
        print(f"")
        print(f"🔧 OPTIMIZATION RECOMMENDATIONS:")
        print(f"  1. Add Redis caching for GET operations")
        print(f"  2. Batch POST operations where possible")
        print(f"  3. Optimize database indexes for search queries")
        print(f"  4. Add connection pooling optimization")
        print(f"  5. Consider read replicas for analytics")
        print(f"")
        print(f"📈 SCALING TARGETS:")
        print(f"  • Production goal: 500+ RPS for GET operations")
        print(f"  • Production goal: 100+ RPS for POST operations")
        print(f"  • Production goal: Handle 1000+ concurrent users")
...
class TestDataLoadedRPS:
    ...
    async def test_rps_with_large_dataset(self):
        ...
        print(f"This test simulates API performance when database contains:")
        print(f"  • 10,000+ items")
        print(f"  • 50+ warehouses") 
        print(f"  • 50,000+ inventory levels")
        print(f"  • Complex search and filtering operations")
        ...
        print(f"📊 Large Dataset RPS Results:")
        print(f"  Target RPS: {target_rps}")
        print(f"  Actual RPS: {actual_rps:.2f}")
        print(f"  Success rate: {success_rate*100:.2f}%")
        print(f"  Test duration: {total_test_time:.2f}s")
        ...
        print(f"  💡 This simulates real-world performance with large datasets")
        
        assert len(successful_requests) > 0
        assert success_rate > 0.7, f"Success rate too low with large dataset: {success_rate*100:.2f}%"
