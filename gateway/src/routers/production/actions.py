...
from .models import (
    Order, ProductionTask, QualityCheck, QualityChecklistItem,
    Resource, TaskResource, OrderStatus, ProductionStage, QualityStatus
)
from .schemas import (
    OrderCreate, OrderUpdate, ProductionTaskCreate, ProductionTaskUpdate,
    QualityCheckCreate, QualityCheckUpdate, ResourceCreate, ResourceUpdate
)
...
class ProductionService:
    ...
    async def create_order(self, order_data: OrderCreate) -> Order:
        ...
        try:
            ...
            logger.info(f"Created order {order_number} for customer {order_data.customer_name}")
            return order
    ...
    async def get_order(self, order_id: uuid.UUID) -> Optional[Order]:
        ...
        try:
            result = await self.db.execute(
                select(Order)
                .options(selectinload(Order.production_tasks), selectinload(Order.quality_checks))
    ...
    async def update_order(self, order_id: uuid.UUID, order_data: OrderUpdate) -> Optional[Order]:
        """Обновляет заказ"""
        try:
    ...
    async def change_order_status(self, order_id: uuid.UUID, new_status: OrderStatus) -> bool:
        ...
        try:
            ...
            elif new_status == OrderStatus.COMPLETED and not order.actual_completion_date:
            ...
            logger.info(f"Changed order {order.order_number} status from {old_status} to {new_status}")
            return True
    ...
    async def create_production_task(self, task_data: ProductionTaskCreate) -> ProductionTask:
        ...
        try:
            ...
            logger.info(f"Created production task '{task.name}' for order {task.order_id}")
            return task
    ...
    async def create_quality_check(self, check_data: QualityCheckCreate) -> QualityCheck:
        ...
        try:
            ...
            logger.info(f"Created quality check '{check_data.check_name}' for order {check_data.order_id}")
    ...
    async def update_quality_check_status(
        ...
        try:
            ...
            if status in [QualityStatus.PASSED, QualityStatus.FAILED, QualityStatus.REWORK]:
    ...
    async def create_resource(self, resource_data: ResourceCreate) -> Resource:
        ...
        try:
            ...
            logger.info(f"Created resource '{resource.name}' of type {resource.resource_type}")
            return resource
    ...
    async def assign_resource_to_task(
        ...
        try:
            ...
            logger.info(f"Assigned resource {resource.name} to task {task_id} for {assigned_hours} hours")
            return True
    ...
    async def get_production_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            in_production_result = await self.db.execute(
                select(func.count(Order.id)).where(Order.status == OrderStatus.IN_PRODUCTION)
            )
            ...
            delayed_result = await self.db.execute(
                select(Order).where(
                    and_(
                        ...
                        Order.status.in_([OrderStatus.IN_PRODUCTION, OrderStatus.QUALITY_CHECK])
                    )
                )
            )
            ...
            avg_time_result = await self.db.execute(
                select(func.avg(
                    func.extract('epoch', Order.actual_completion_date - Order.actual_start_date) / 86400
            ...
            quality_stats_result = await self.db.execute(
                select(
                    ...
                    func.count(QualityCheck.id).filter(QualityCheck.status == QualityStatus.PASSED)
                )
            )
            quality_stats = quality_stats_result.first()
            total_checks = quality_stats[0] or 0
            passed_checks = quality_stats[1] or 0
            quality_pass_rate = (passed_checks / total_checks * 100) if total_checks > 0 else 0
    ...
    async def get_order_timeline(self, order_id: uuid.UUID) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if order.actual_start_date and order.actual_completion_date:
                total_duration = (order.actual_completion_date - order.actual_start_date).days
            elif order.planned_start_date and order.planned_completion_date:
                total_duration = (order.planned_completion_date - order.planned_start_date).days
            
            # Проверяем соответствие графику
            is_on_schedule = True
            if order.planned_completion_date and order.actual_completion_date:
                is_on_schedule = order.actual_completion_date <= order.planned_completion_date
