...
class Order(Base):
    ...
    current_stage: Mapped[ProductionStage] = mapped_column(Enum(ProductionStage), default=ProductionStage.PLANNING)
    ...
    # Связи
    production_tasks: Mapped[List["ProductionTask"]] = relationship("ProductionTask", back_populates="order")
    quality_checks: Mapped[List["QualityCheck"]] = relationship("QualityCheck", back_populates="order")
...
class ProductionTask(Base):
    ...
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    stage: Mapped[ProductionStage] = mapped_column(Enum(ProductionStage), nullable=False)
    ...
    assigned_resources: Mapped[List["TaskResource"]] = relationship("TaskResource", back_populates="task")
...
class QualityCheck(Base):
    ...
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    checklist_items: Mapped[List["QualityChecklistItem"]] = relationship("QualityChecklistItem", back_populates="quality_check")
    
    # Статус и результаты
    status: Mapped[QualityStatus] = mapped_column(Enum(QualityStatus), default=QualityStatus.PENDING)
...
class QualityChecklistItem(Base):
    ...
    id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    quality_check_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
    ...
    # Связи
    quality_check: Mapped["QualityCheck"] = relationship("QualityCheck", back_populates="checklist_items")
...
class Resource(Base):
    ...
    # Характеристики
    capacity: Mapped[Optional[float]] = mapped_column(Float, nullable=True)  # Производительность/мощность
    unit: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # Единица измерения
    cost_per_hour: Mapped[Optional[float]] = mapped_column(Float, nullable=True)  # Стоимость в час
    ...
    # Связи
    task_assignments: Mapped[List["TaskResource"]] = relationship("TaskResource", back_populates="resource")
...
class TaskResource(Base):
    ...
    resource_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
    ...
    # Связи
    task: Mapped["ProductionTask"] = relationship("ProductionTask", back_populates="assigned_resources")
    resource: Mapped["Resource"] = relationship("Resource", back_populates="task_assignments")
