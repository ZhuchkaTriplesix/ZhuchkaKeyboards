...
from sqlalchemy import types, String, Boolean, Integer, Float, DateTime, Text, Enum, Numeric
...
class Item(Base):
    ...
    unit_of_measure: Mapped[UnitOfMeasure] = mapped_column(Enum(UnitOfMeasure), nullable=False)
    weight_kg: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    dimensions: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # "LxWxH см"
    ...
    is_tracked: Mapped[bool] = mapped_column(Boolean, default=True)  # Отслеживается ли количество
    
    # Связи
    inventory_levels: Mapped[List["InventoryLevel"]] = relationship("InventoryLevel", back_populates="item")
    transactions: Mapped[List["InventoryTransaction"]] = relationship("InventoryTransaction", back_populates="item")
    supplier_items: Mapped[List["SupplierItem"]] = relationship("SupplierItem", back_populates="item")
...
class Warehouse(Base):
    ...
    # Связи
    inventory_levels: Mapped[List["InventoryLevel"]] = relationship("InventoryLevel", back_populates="warehouse")
...
class WarehouseZone(Base):
    ...
    id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    warehouse_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
    ...
    # Характеристики зоны
    zone_type: Mapped[str] = mapped_column(String(100), nullable=False)  # "storage", "receiving", "shipping", etc.
    ...
    inventory_levels: Mapped[List["InventoryLevel"]] = relationship("InventoryLevel", back_populates="zone")
...
class InventoryLevel(Base):
    ...
    warehouse_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
    zone_id: Mapped[Optional[UUID]] = mapped_column(PostgresUUID(as_uuid=True), nullable=True)
    
    # Количества
    current_quantity: Mapped[int] = mapped_column(Integer, default=0)
    reserved_quantity: Mapped[int] = mapped_column(Integer, default=0)  # Зарезервировано
    available_quantity: Mapped[int] = mapped_column(Integer, computed="current_quantity - reserved_quantity")
    
    # Локация
    location_code: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # Код места хранения
    bin_location: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # Номер ячейки
    ...
    zone: Mapped[Optional["WarehouseZone"]] = relationship("WarehouseZone", back_populates="inventory_levels")
...
class InventoryTransaction(Base):
    ...
    unit_cost: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    total_cost: Mapped[Optional[float]] = mapped_column(Float, computed="quantity * unit_cost")
    
    # Ссылки на документы
    reference_number: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # Номер документа
    reference_type: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # Тип документа
...
class Supplier(Base):
    ...
    # Связи
    supplier_items: Mapped[List["SupplierItem"]] = relationship("SupplierItem", back_populates="supplier")
...
class SupplierItem(Base):
    ...
    id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    supplier_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
    item_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
...
class PurchaseOrder(Base):
    ...
    # Статус
    status: Mapped[str] = mapped_column(String(50), default="draft")  # draft, sent, confirmed, received, cancelled
    ...
    po_items: Mapped[List["PurchaseOrderItem"]] = relationship("PurchaseOrderItem", back_populates="purchase_order")
...
class PurchaseOrderItem(Base):
    ...
    id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    purchase_order_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
    item_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), nullable=False)
    ...
    is_fully_received: Mapped[bool] = mapped_column(Boolean, computed="received_quantity >= quantity")
    
    # Связи
    purchase_order: Mapped["PurchaseOrder"] = relationship("PurchaseOrder", back_populates="po_items")
