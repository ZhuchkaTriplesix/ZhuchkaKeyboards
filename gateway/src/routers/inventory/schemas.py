...
from routers.inventory.models import ItemType, ItemCategory, UnitOfMeasure, SupplierStatus
...
class ItemBase(BaseModel):
    ...
    max_stock_level: Optional[int] = Field(None, ge=0, description="Максимальный уровень запасов")
    reorder_point: Optional[int] = Field(None, ge=0, description="Точка перезаказа")
    lead_time_days: Optional[int] = Field(None, ge=0, description="Время поставки в днях")
...
class InventoryLevelBase(BaseModel):
    ...
    location_code: Optional[str] = Field(None, max_length=100, description="Код места хранения")
    bin_location: Optional[str] = Field(None, max_length=100, description="Номер ячейки")
...
class StockMovementRequest(BaseModel):
    ...
    reason: str = Field(..., min_length=1, max_length=200, description="Причина движения")
    reference_number: Optional[str] = Field(None, max_length=100, description="Номер документа")
