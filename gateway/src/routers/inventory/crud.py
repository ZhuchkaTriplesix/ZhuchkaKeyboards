...
from routers.inventory.models import (
    Item,
    Warehouse,
    InventoryLevel,
...
async def get_item_by_sku(session: AsyncSession, sku: str) -> Optional[Item]:
    ...
    result = await session.execute(select(Item).where(Item.sku == sku))
    return result.scalar_one_or_none()
...
async def update_item(
    session: AsyncSession, item_id: UUID, item_data: ItemUpdate
) -> Optional[Item]:
    """Обновить товар"""
...
async def search_items(
    session: AsyncSession, filters: ItemSearchFilter, page: int = 1, size: int = 20
) -> Dict[str, Any]:
...
async def update_warehouse(
    session: AsyncSession, warehouse_id: UUID, warehouse_data: WarehouseUpdate
) -> Optional[Warehouse]:
    """Обновить склад"""
...
async def get_inventory_level(
    session: AsyncSession, item_id: UUID, warehouse_id: UUID
    ...
    result = await session.execute(
        ...
        .options(
            selectinload(InventoryLevel.item), selectinload(InventoryLevel.warehouse)
        )
    )
    return result.scalar_one_or_none()
...
async def create_inventory_level(
    session: AsyncSession, level_data: InventoryLevelCreate
) -> InventoryLevel:
...
async def search_inventory_levels(
    session: AsyncSession, filters: InventorySearchFilter, page: int = 1, size: int = 20
) -> Dict[str, Any]:
...
async def move_stock(
    session: AsyncSession, movement_data: StockMovementRequest
) -> InventoryLevel:
    """Движение товара на складе"""
    # Получаем или создаем уровень запасов
    level = await get_inventory_level(
        session, movement_data.item_id, movement_data.warehouse_id
    )
    ...
    if not level:
        ...
        if new_quantity < 0:
            raise ValueError(
                f"Недостаточно товара на складе. Доступно: {level.current_quantity}"
            )
...
async def reserve_stock(
    session: AsyncSession, item_id: UUID, warehouse_id: UUID, quantity: int
) -> bool:
...
async def get_low_stock_items(
    ...
    query = query.group_by(Item.id, Item.sku, Item.name, Item.min_stock_level).having(
...
async def get_inventory_summary(
    ...
    total_items_result = await session.execute(
        select(func.count(func.distinct(InventoryLevel.item_id))).select_from(
            base_query.subquery()
        )
    )
    ...
    total_quantity_result = await session.execute(
        select(func.sum(InventoryLevel.current_quantity)).select_from(
            base_query.subquery()
        )
    )
    ...
    reserved_quantity_result = await session.execute(
        select(func.sum(InventoryLevel.reserved_quantity)).select_from(
            base_query.subquery()
        )
    )
