...
async def create_item(item_data: ItemCreate, session: DbSession):
    """Создать новый товар"""
...
async def update_item(item_id: UUID, item_data: ItemUpdate, session: DbSession):
    """Обновить товар"""
...
async def delete_item(item_id: UUID, session: DbSession):
...
async def create_warehouse(warehouse_data: WarehouseCreate, session: DbSession):
    """Создать новый склад"""
...
async def get_warehouse(warehouse_id: UUID, session: DbSession):
    """Получить склад по ID"""
    warehouse = await crud.get_warehouse_by_id(session, warehouse_id)
    if not warehouse:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Склад не найден"
        )
...
async def update_warehouse(
    warehouse_id: UUID, warehouse_data: WarehouseUpdate, session: DbSession
):
    """Обновить склад"""
...
async def get_inventory_level(item_id: UUID, warehouse_id: UUID, session: DbSession):
    """Получить уровень запасов товара на складе"""
    level = await crud.get_inventory_level(session, item_id, warehouse_id)
    if not level:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Уровень запасов не найден"
        )
...
async def create_inventory_level(level_data: InventoryLevelCreate, session: DbSession):
...
async def move_stock(movement_data: StockMovementRequest, session: DbSession):
...
async def release_reservation(
    ...
    session: DbSession,
    quantity: int = Query(
        ..., ge=1, description="Количество для снятия резервирования"
    ),
):
...
async def get_low_stock_items(
    ...
    warehouse_id: Optional[UUID] = Query(
        None, description="ID склада (если не указан, поиск по всем складам)"
    ),
):
...
async def get_inventory_summary(
    session: DbSession,
    warehouse_id: Optional[UUID] = Query(
        None, description="ID склада (если не указан, сводка по всем складам)"
    ),
):
