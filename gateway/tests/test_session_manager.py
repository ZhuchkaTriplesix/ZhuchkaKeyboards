...
class TestSessionManager:
    ...
    @pytest.mark.asyncio
    async def test_create_session_success(self, session_manager_instance, mock_user_data):
    ...
    @pytest.mark.asyncio
    async def test_create_session_max_sessions_limit(self, session_manager_instance, mock_user_data):
        """Тест ограничения максимального количества сессий"""
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            # Симулируем максимальное количество активных сессий
            mock_redis.smembers.return_value = ['session1', 'session2', 'session3', 'session4', 'session5']
    ...
    @pytest.mark.asyncio
    async def test_get_session_success(self, session_manager_instance, mock_session_data):
        """Тест успешного получения сессии"""
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            mock_redis.get_record.return_value = '{"user_id": "test-user-123", "is_active": true}'
            mock_redis.set_record.return_value = True
    ...
    @pytest.mark.asyncio
    async def test_revoke_session_success(self, session_manager_instance, mock_session_data):
    ...
    async def test_revoke_session_not_found(self, session_manager_instance):
        ...
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            ...
            result = await session_manager_instance.revoke_session('non-existent-session')
    ...
    async def test_revoke_all_user_sessions(self, session_manager_instance):
        ...
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            ...
            with patch.object(session_manager_instance, 'revoke_session', return_value=True):
                result = await session_manager_instance.revoke_all_user_sessions('test-user-123')
            
            assert result == 3
    
    @pytest.mark.asyncio
    async def test_validate_session_success(self, session_manager_instance, mock_session_data):
        """Тест успешной валидации сессии"""
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            mock_redis.get_record.return_value = '{"is_active": true, "last_activity": "2023-12-01T12:00:00"}'
            mock_redis.set_record.return_value = True
            
            session = await session_manager_instance.validate_session('test-session-123')
    ...
    async def test_validate_session_expired(self, session_manager_instance):
        ...
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            mock_redis.get_record.return_value = f'{{"is_active": true, "last_activity": "{expired_time}"}}'
            mock_redis.del_record.return_value = True
            mock_redis.srem.return_value = 1
    ...
    async def test_validate_session_inactive(self, session_manager_instance):
        ...
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            mock_redis.get_record.return_value = '{"is_active": false, "last_activity": "2023-12-01T12:00:00"}'
    ...
    async def test_get_user_by_session_success(self, session_manager_instance):
        ...
        with patch('services.redis.rediska.redis_manager') as mock_redis:
            ...
            with patch.object(session_manager_instance, 'validate_session') as mock_validate:
                mock_validate.return_value = {'user_id': 'test-user-123'}
                
                with patch('sqlalchemy.ext.asyncio.AsyncSession') as mock_db:
                    mock_db.execute.return_value.scalar_one_or_none.return_value = mock_user
                    
                    user = await session_manager_instance.get_user_by_session('test-session-123', mock_db)
    ...
    async def test_get_user_by_session_invalid_session(self, session_manager_instance):
        ...
        with patch.object(session_manager_instance, 'validate_session', return_value=None):
            with patch('sqlalchemy.ext.asyncio.AsyncSession') as mock_db:
                user = await session_manager_instance.get_user_by_session('invalid-session', mock_db)
    ...
    async def test_get_user_by_session_user_not_found(self, session_manager_instance):
        ...
        with patch.object(session_manager_instance, 'validate_session') as mock_validate:
            ...
            with patch('sqlalchemy.ext.asyncio.AsyncSession') as mock_db:
                ...
                with patch.object(session_manager_instance, 'revoke_session', return_value=True) as mock_revoke:
                    user = await session_manager_instance.get_user_by_session('test-session-123', mock_db)
